	...
	public void exec() {
		...
		try{
			...
			if(StringUtil.isEmpty(ownerId)){
				...
				if(!ownerId.equals(AccountInitUtil.model_property_owner_id)){
					reload_property = true;
					AccountInitUtil.model_property_owner_id = ownerId;
					logger.info("{} 系统变量有修改,重新加载:{}",trade_id,ownerId);
				}else{
					logger.info("{} 系统变量无更新,不需加载",trade_id);
				}
				if(reload_property){
					List<Map<String, Object>> property_list =propertyService.queryProperties(AccountInitUtil.model_id);
